...
func _physics_process(delta: float) -> void:
	...
	if state in [States.TAKE_OFF, States.LANDING] and distance_to_closest_planet() > landing_distance:
		state = States.FLYING
	elif state in [States.FLYING, States.TAKE_OFF]:
		handle_player_input()
	elif state == States.LANDING:
		# Temporarily Interrupt user input
		reposition_for_landing(delta)
